...
class Kismet_MCP_Servers_Strategies {
    ...
    public function get_ordered_strategies() {
        ...
        error_log("KISMET MCP SERVERS STRATEGIES: Admin toggle status - should_send_events: " . ($should_send_events ? 'TRUE' : 'FALSE'));
        
        // **IF CHECKBOX IS UNCHECKED (should send events), prioritize metrics-enabled strategy**
        if ($should_send_events) {
            error_log("KISMET MCP SERVERS STRATEGIES: Prioritizing metrics-enabled strategy (checkbox unchecked)");
            ...
            error_log("KISMET MCP SERVERS STRATEGIES: Final strategy order (events enabled): " . implode(', ', $base_strategies));
            return $base_strategies;
        }
        
        // **IF CHECKBOX IS CHECKED (static files only), keep original strategy order**
        error_log("KISMET MCP SERVERS STRATEGIES: Using original strategy order (checkbox checked - static files only)");
        $original_strategies = $this->get_base_strategies_for_server();
        error_log("KISMET MCP SERVERS STRATEGIES: Final strategy order (static files only): " . implode(', ', $original_strategies));
