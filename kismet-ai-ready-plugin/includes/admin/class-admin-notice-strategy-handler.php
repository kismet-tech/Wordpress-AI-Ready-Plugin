<?php
/**
 * Kismet Strategy Switcher & Admin Notice Handler
 * 
 * WHAT THIS ADDS TO THE UI:
 * - "Try [Strategy]" buttons in admin notices when endpoints fail
 * - Success/error messages after strategy switches
 * - Strategy status info ("via Static File", "WP Rewrite not working", etc.)
 * 
 * WHERE USERS SEE THIS:
 * - Admin notices at the top of WordPress admin pages
 * - Appears across ALL admin pages when endpoints are failing
 * - NOT on the main settings page - this is for site-wide notifications
 * 
 * HOW IT WORKS:
 * 1. Endpoint fails → Notice shows "Static File not working" 
 * 2. User clicks "Try WP Rewrite" button
 * 3. This class handles the switch securely via WordPress admin actions
 * 4. Plugin deactivates/reactivates to implement the change
 * 5. Success message appears: "✅ Successfully switched to WP Rewrite"
 * 
 * TECHNICAL DETAILS:
 * - Uses WordPress admin actions (not AJAX) for security
 * - Generates nonce-protected URLs for strategy switch buttons
 * - Stores strategy preferences in WordPress options
 * - Handles plugin lifecycle (deactivate/reactivate) to apply changes
 * 
